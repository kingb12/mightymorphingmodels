package mightymorphingmodels::mightymorphingmodelsClient;
...
mightymorphingmodels::mightymorphingmodelsClient
...
sub new
{
    ...
    my $self = {
	client => mightymorphingmodels::mightymorphingmodelsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function morph_model (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to morph_model:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'morph_model');
	}
    }
	...
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method morph_model",
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "mightymorphingmodels.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method morph_model",
            status_line => $self->{client}->status_line,
            method_name => 'morph_model',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for mightymorphingmodels::mightymorphingmodelsClient\n";
    }
    if ($sMajor == 0) {
        warn "mightymorphingmodels::mightymorphingmodelsClient version is $svr_version. API subject to change.\n";
    }
}
...
package mightymorphingmodels::mightymorphingmodelsClient::RpcClient;
